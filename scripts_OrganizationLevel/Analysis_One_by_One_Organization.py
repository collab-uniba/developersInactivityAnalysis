...
import ActivitiesExtractor_Organization as ae
import logging
...
def runActivitiesExtractionRoutine(g, super_path, repo, project_name, project_start_dt, chosen_organization, active_users):  
    ...
    ### Get Activities from the Main Project
    ae.get_repo_activities(g, path, repo, project_url, project_start_dt, active_users)
...
main_project_path = super_path+'/'+main_project
active_users_df=pandas.read_csv(main_project_path+'/active_users.csv', sep=';')
active_users=active_users_df.id.tolist()

### Get Activities from the Main Project
cfg.waitRateLimit(g)
repo = g.get_repo(chosen_organization+'/'+main_project)
runActivitiesExtractionRoutine(g, super_path, repo, main_project, project_start_dt, chosen_organization, active_users)

### Get Activities from Other Organization's Projects
cfg.checkRateLimit(g)
...
for repo in org_repos:
    ...
    if project_name != main_project:
        runActivitiesExtractionRoutine(g, super_path, repo, project_name, project_start_dt, chosen_organization, active_users)

#Read Breaks Table
with open(super_path+'/inactivity_interval_list.csv', 'r') as f:  #opens PW file
...
for index, row in active_users_breaks.iterrows():
        ...
        for j in range(w_start,w_end+1):
            ...
            if((duration>threshold) & (date_range not in longer_breaks.datelimits.tolist())):
                ...
                if len(user_actions)>0:
                    #brake_actions.index=user_actions.action # Set the action name as a index (Commented Because the Index is set before)
    ...
    path = (super_path+"/Sleeping&Awaken_Users/Details")
    os.makedirs(path, exist_ok=True) 
...
path = (super_path+"/Sleeping&Awaken_Users")
...
project_transitions=util.countDevTransitions(super_path, breaks_df['durations'])
durations = util.reportDevsBreaksLengthDistribution(super_path)
