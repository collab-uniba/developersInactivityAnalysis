...
for i in range(START_FROM, len(p_names)+1):### Starts from 1 because of the token
    ...
    active_users_df.to_csv(path+"/active_users.csv", sep=';', encoding='utf-8', na_rep='NA', header=True, index=False, mode='w', quoting=None, quotechar='"', line_terminator='\n', decimal='.')
    ...
    ### --------------- NEEDED ONLY TO KEEP TRACK OF ACTIVE USERS IN THE MAIN PROJECT
